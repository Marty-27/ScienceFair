...
public class SolarMovementProvider : LocomotionProvider
{
    public SolarSystemController solarSystemController;
    public float speed = 1.5f;
    // Removed gravityMultiplier and gravity since we don't need gravity
    ...
    private void Update()
    {
        if (!solarSystemController.IsHovering && !solarSystemController.isFocusing)
        {
    ...
    {
        Vector3 verticalMove = Vector3.down;
        characterController.Move(verticalMove);
        if (rig.transform.position.y > 1)
        {
            downBtn.SetActive(true);
